...
class SparkMLPipelineStrategy(PipelineStrategy):
    ...
    def fit_transform(self, df, **fit_params):
        ...
        if transformed_data.select('features').head().features.size != expected_num_features:
            sys.stdout.write(f"Transformed data has {transformed_data.select('features').head().features.size} features, but the model expects {expected_num_features} features.")
