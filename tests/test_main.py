...
def test_score_success(mock_postgres_saver, mock_save_metrics_to_json, mock_logger, mock_traceability_logger, mock_pipeline_handler, mock_get_current_timestamp, mock_traceability):
    """
    ...
    mock_pipeline_handler_instance.get_predictions_and_metrics.return_value = (['pred1', 'pred2'], {'metric1': 0.9}, 'result', 'data')
    mock_traceability.get_run_info.return_value = {'info': 'test_info'}

    result = score()

    assert result == {"predictions": ['pred1', 'pred2'], "summary": 'result'}
    mock_logger.log_success.assert_called_once_with(message="Model scored successfully")
